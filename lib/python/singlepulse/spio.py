...
def read_RRATrap_info(groupfile, group_to_read, rank):
    ...
    """
    group_to_read=int(group_to_read)
    ...
    if files[rank-1] != "Number of rank %i groups: 0 "%rank: # checks whether there are %i \
        ...
        dm_arr,sigma_arr,width_arr=[],[],[]
        for l in arr_2:
            try:
                dm_arr.append(float(l[0]))
                sigma_arr.append(float(l[1]))
                width_arr.append(float(l[4]))
            except:
                pass
            dm_arr=_np.array(dm_arr)
            sigma_arr=_np.array(sigma_arr)
            width_arr=_np.array(width_arr)
    else:
